class PersonalLifeController < ApplicationController
  ...
  def edit
    ...
    # @pd_user.personal_life keeps its current value unless it's nil (i.e., keep current
    # personal_life if there is one). If it is nil, give it is nil create a new personal_life
    @pd_user.personal_life ||= PersonalLife.new
    
    @personal_life = @pd_user.personal_life
    
    if (param_posted?(:personal_life))
      if (@pd_user.personal_life.update_attributes(params[:personal_life]))
